...
export default function SalesModule({ state, updateState }: { state: AppState, updateState: (s: Partial<AppState>) => void }) {
  ...
  setShowDetails({ ...(d as any), ventaDetalle: sale });
      ...
      {showMultiModal && (<FloatingPaymentModal total={totalBS} totalCents={Math.round(totalBS * 100)} exchangeRate={state.tasa} onClose={() => setShowMultiModal(false)} onConfirm={(data) => { ejecutarVenta(data.payments.map(p => ({ metodo: p.method as PaymentMethod, montoUSD: Number(p.usdAmountCents != null ? (p.usdAmountCents / 100) : (Math.round((p.amountCents * 100) / Math.round(state.tasa * 100)) / 100)), montoBS: p.amount }))); setShowMultiModal(false); }} />)}
