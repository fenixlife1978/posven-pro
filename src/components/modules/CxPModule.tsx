...
export default function CxPModule({ state, updateState }: CxPModuleProps) {
  ...
  const handleProcessPayment = () => {
    ...
    toast({ title: "Pago registrado", description: `Se ha registrado el pago de ${Utils.fmtUSD(amount)}` });
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-black text-primary uppercase italic tracking-tighter">Cuentas por Pagar</h2>
          <p className="text-[10px] text-ink font-bold uppercase tracking-widest opacity-60">Control de Obligaciones con Proveedores</p>
        </div>
        <button onClick={() => exportarPDFCxP(pendientes, state.empresa, totalPendiente)} className="btn btn-secondary h-11 px-6 font-black uppercase text-xs shadow-md flex items-center gap-2">
          <FileText className="w-4 h-4" /> Reporte de Pagos
        </button>
      </div>
        ...
        <div className="table-wrap">
          <table className="w-full">
            ...
            <tbody className="bg-white">
              {state.cxp.length === 0 ? (
                <tr><td colSpan={7} className="text-center py-24 text-ink/20 font-black uppercase italic tracking-widest">No se registran cuentas por pagar</td></tr>
              ) : (
                state.cxp.map(x => (
                  <tr key={x.id} className="border-b border-line/40 hover:bg-surface-warm/20 transition-colors">
                    ...
                    <td className={`text-xs font-bold py-4 ${x.fechaVencimiento < Utils.hoy() && x.estado !== 'pagada' ? 'text-status-danger' : 'text-ink'}`}>{Utils.fmtFecha(x.fechaVencimiento)}</td>
                    ...
                    <td className="py-4 px-6 text-center">
                       <div className="flex justify-center gap-1">
                          <button onClick={() => setShowDetails(x)} className="btn-icon h-8 w-8 text-ink hover:text-brand-gold" title="Ver Detalle"><Eye className="w-4 h-4"/></button>
                          {x.estado !== 'pagada' && (
      ...
      {showDetails && (
        <div className="modal show"><div className="modal-bg" onClick={() => setShowDetails(null)}></div>
          <div className="modal-box max-w-[600px] bg-white border-2 border-line rounded-xl overflow-hidden shadow-2xl">
            ...
            <div className="modal-body p-6 bg-white space-y-6 max-h-[75vh] overflow-y-auto">
              <div className="grid grid-cols-2 gap-4">
                 <div className="p-3 bg-surface-soft rounded-lg border border-line"><label className="text-[8px] font-black uppercase text-ink/50 block mb-1">Factura Original</label><p className="text-lg font-black text-ink">{fmt4(showDetails.montoUSD)}</p></div>
                 <div className="p-3 bg-brand-gold-soft border border-brand-gold/20 rounded-lg"><label className="text-[8px] font-black uppercase text-brand-gold-deep block mb-1">Saldo por Pagar</label><p className="text-lg font-black text-brand-gold-deep">{fmt4(showDetails.saldoUSD)}</p></div>
              </div>
              <div className="space-y-3">
                 ...
                 <div className="max-h-[200px] overflow-y-auto space-y-2 pr-1">
                    {(!showDetails.historialPagos || showDetails.historialPagos.length === 0) ? (
                      <div className="py-10 text-center text-ink/20 font-black uppercase italic text-[10px]">No hay abonos registrados</div>
                    ) : (
                      showDetails.historialPagos.map((p: any, idx: number) => (
                        <div key={idx} className="flex justify-between items-center p-3 bg-surface-soft border border-line rounded-lg">
                           <div className="space-y-0.5"><p className="text-[10px] font-black uppercase">{Utils.fmtFecha(p.fecha)}</p><p className="text-[8px] font-bold text-ink/40 mono">{p.reciboId}</p></div>
                           <div className="text-right"><p className="text-xs font-black text-status-success">+{Utils.fmtUSD(p.montoUSD)}</p><p className="text-[8px] font-black text-ink/40 uppercase">{Utils.metodoLabel(p.metodo)}</p></div>
                        </div>
                      ))
                    )}
                 </div>
      ...
      {showPaymentModal && (
        <div className="modal show"><div className="modal-bg" onClick={() => setShowPaymentModal(null)}></div>
          <div className="modal-box bg-white max-w-sm border-2 border-line rounded-2xl overflow-hidden shadow-2xl">
            <div className="modal-head py-4 px-6 bg-ink flex justify-between items-center">
            ...
            <div className="modal-body p-8 space-y-6">
               <div className="bg-surface-soft p-8 rounded-[20px] text-center border border-line shadow-inner">
                  <p className="text-ink/40 text-[9px] font-black uppercase mb-2">SALDO PENDIENTE</p>
                  <p className="text-3xl font-black text-status-danger">{fmt4(showPaymentModal.saldoUSD)}</p>
               </div>
               <div className="form-group"><label className="text-ink text-[10px] font-black uppercase mb-1">METODO</label>
                 <select className="form-select h-12 text-sm font-black uppercase" value={paymentMethod} onChange={e => setPaymentMethod(e.target.value as any)}>
                    <option value="efectivo_usd">Efectivo USD</option><option value="efectivo_bs">Efectivo BS</option><option value="pagomovil">Pago Movil</option><option value="zelle">Zelle</option>
                  </select>
               </div>
               <div className="form-group"><label className="text-ink text-[10px] font-black uppercase mb-1">MONTO A PAGAR (USD)</label>
                   <input className="form-input h-12 text-xl font-black" type="number" value={paymentAmount} onChange={e => setPaymentAmount(e.target.value)} />
               </div>
               <button onClick={handleProcessPayment} className="btn btn-primary w-full h-14 font-black uppercase text-xs shadow-xl">CONFIRMAR PAGO</button>
            </div>
          </div>
        </div>
      )}
