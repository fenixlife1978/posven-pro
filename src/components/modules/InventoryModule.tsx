...
import { 
  Plus, Search, Edit2, Trash2, Boxes, X, History, Layers, 
  ShoppingBag, TrendingUp, Box, ClipboardList, Info, 
  PlusCircle, MinusCircle, Check, Filter, PackageCheck, AlertCircle, FileText
} from 'lucide-react';
        ...
        <div className="space-y-6 animate-in fade-in duration-500">
              ...
              <CardContent className="pt-6">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-2xl font-black text-white">{prods.length}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            <div className="flex gap-2 w-full md:w-auto">
              <Button variant="outline" className="flex-1 md:flex-none gap-2 border-[#2a2a2a] text-white hover:bg-[#2a2a2a]" onClick={() => generarPDFInventarioSimple(prods, state.empresa)}>
                <FileText className="w-4 h-4" /> PDF
              </Button>
              <Button className="flex-1 md:flex-none bg-[#c8952e] hover:bg-[#d9a540] text-black font-black gap-2" onClick={() => setShowProducto('nuevo')}>
                <Plus className="w-4 h-4" /> Nuevo Ítem
              </Button>
            </div>
          </div>

          <Card className="overflow-hidden bg-[#131313] border-[#2a2a2a] shadow-xl">
            <div className="table-wrap">
              <Table>
                <TableHeader className="bg-[#0b0b0b]">
                  <TableRow className="border-[#2a2a2a] hover:bg-transparent">
                    <TableHead className="text-white/70 font-bold uppercase text-[10px]">Código</TableHead>
                    <TableHead className="text-white/70 font-bold uppercase text-[10px]">Nombre Producto</TableHead>
                    <TableHead className="text-white/70 font-bold uppercase text-[10px]">Categoría</TableHead>
                    <TableHead className="text-white/70 font-bold uppercase text-[10px] text-right">Costo ($)</TableHead>
                    <TableHead className="text-white/70 font-bold uppercase text-[10px] text-right">Venta ($)</TableHead>
                    <TableHead className="text-white/70 font-bold uppercase text-[10px] text-center">Stock</TableHead>
                    <TableHead className="text-white/70 font-bold uppercase text-[10px] text-center">Acciones</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {prods.length === 0 ? (
                    <TableRow><TableCell colSpan={7} className="text-center py-20 text-white/20 font-black italic uppercase">No se encontraron productos</TableCell></TableRow>
                  ) : (
                    prods.map(p => (
                      <TableRow key={p.id} className="border-[#2a2a2a] hover:bg-[#1a1a1a] transition-colors">
                        <TableCell className="mono text-xs font-black text-[#c8952e]">{p.codigo}</TableCell>
                        <TableCell className="font-bold text-white uppercase">
                          <div className="flex items-center gap-2">
                            {p.isKit && <Layers className="w-3.5 h-3.5 text-[#c8952e]" />}
                            {p.nombre}
                          </div>
                        </TableCell>
                        <TableCell><span className="badge bg-[#2a2a2a] text-white/70 font-black px-2 py-0.5 rounded text-[10px] uppercase">{p.categoria}</span></TableCell>
                        <TableCell className="mono font-bold text-white/50 text-right">{Utils.fmtUSD(p.costoUSD)}</TableCell>
                        <TableCell className="mono text-[#c8952e] font-black text-right">{Utils.fmtUSD(p.precioUSD)}</TableCell>
                        <TableCell className="text-center">
                          <Badge variant={p.stock <= (p.stockMinimo || 0) ? 'destructive' : 'secondary'} className={p.stock <= (p.stockMinimo || 0) ? 'bg-red-500/20 text-red-500' : 'bg-[#2a2a2a] text-white/70'}>
                            {p.stock}
                          </Badge>
                        </TableCell>
                        <TableCell>
                          <div className="flex justify-center gap-1">
                            <button className="btn-icon h-8 w-8 text-[#c8952e] hover:bg-[#c8952e]/10" onClick={() => setShowProducto(p.id)}><Edit2 className="w-4 h-4" /></button>
                            <button className="btn-icon h-8 w-8 text-blue-400 hover:bg-blue-400/10" onClick={() => { setSelectedKardexId(p.id); setActiveTab('kardex'); }}><History className="w-4 h-4" /></button>
                            <button className="btn-icon h-8 w-8 text-green-400 hover:bg-green-400/10" onClick={() => setShowAjuste(p.id)}><Boxes className="w-4 h-4" /></button>
                            <button className="btn-icon h-8 w-8 text-red-500 hover:bg-red-500/10" onClick={() => eliminar(p.id)}><Trash2 className="w-4 h-4" /></button>
                          </div>
                        </TableCell>
                      </TableRow>
                    ))
...
function ReporteGeneral({ state }: { state: AppState }) {
  const [groupBy] = useState<'categoria' | 'departamento' | 'proveedor'>('categoria');
      ...
      <Card className="shadow-lg border-line rounded-xl overflow-hidden bg-white">
        <div className="card-head bg-ink border-b border-white/10 px-6 py-4 flex justify-between items-center text-white">
          <h3 className="font-black text-xs uppercase italic tracking-tighter flex items-center gap-2">
            <Boxes className="w-5 h-5 text-[#c8952e]" /> INVENTARIO VALORIZADO ACTUAL
          </h3>
            ...
            <TableBody>
              {filteredProducts.map(p => (
                <TableRow key={p.id} className="border-b border-line/30">
                  ...
                  <TableCell className="mono text-right text-[#c8952e] font-black">{Utils.fmtUSD(p.precioUSD)}</TableCell>
    ...
    <div className="space-y-4">
      <div className="card p-5 bg-white border-line flex flex-wrap gap-4 items-end shadow-sm no-print">
        <div className="flex items-center gap-3 bg-surface-soft p-1 rounded-lg border border-line">
           ...
           <button onClick={() => setUseDates(true)} className={`px-4 py-1.5 rounded-md text-[10px] font-black uppercase transition-all ${useDates ? 'bg-[#c8952e] text-white' : 'text-ink/40'}`}>Periodo</button>
        </div>
        {useDates && (
           <div className="flex items-center gap-2">
              ...
              {groupedVentas.length === 0 ? (
                ...
                groupedVentas.map((g, idx) => (
                  <TableRow key={idx} className="border-b border-line/30 hover:bg-surface-warm/20 transition-colors">
                    ...
                    <TableCell className="text-right font-black text-[#c8952e] mono">{Utils.fmtUSD(g.totalUSD)}</TableCell>
                  </TableRow>
                ))
              )}
            </TableBody>
            ...
            <TableBody>
               {devoluciones.map(d => (
                 <TableRow key={d.id} className="border-b border-line/30">
                   ...
                   <TableCell className="text-right font-black text-red-500">{Utils.fmtUSD(d.totalUSD)}</TableCell>
          ...
          <TableBody>
            {ajustes.map(m => (
              <TableRow key={m.id} className="border-b border-line/30">
                ...
                <TableCell className={`text-center font-black ${m.cantidad > 0 ? 'text-green-600' : 'text-red-500'}`}>{m.cantidad}</TableCell>
          ...
          <TableBody>
            {consumos.map(m => (
              <TableRow key={m.id} className="border-b border-line/30">
                ...
                <TableCell className={`text-center font-black text-red-500`}>{Math.abs(m.cantidad)}</TableCell>
...
function ReporteKardex({ state, selectedId, onSelect }: { state: AppState, selectedId: string | null, onSelect: (id: string) => void }) {
  ...
  return (
    <div className="space-y-4">
      <Card className="p-5 bg-white border-line shadow-sm">
        ...
        <div className="relative">
          ...
          {matches.length > 0 && (
            <div className="absolute top-full left-0 right-0 bg-white border border-line shadow-2xl z-50 mt-1 rounded-xl overflow-hidden">
              {matches.map(p => (
                <div key={p.id} className="p-4 border-b border-line hover:bg-[#c8952e]/10 cursor-pointer transition-all flex justify-between items-center" onClick={() => { onSelect(p.id); setSearch(''); }}>
                  <div><p className="font-black uppercase text-xs">{p.nombre}</p><p className="text-[10px] opacity-40">{p.codigo}</p></div>
                  <Check className="w-4 h-4 text-[#c8952e]" />
                </div>
              ))}
            </div>
          )}
      ...
      {selectedProd && (
        <Card className="overflow-hidden shadow-lg border-line rounded-xl bg-white">
          <div className="card-head bg-ink text-white px-6 py-3 flex justify-between items-center">
            <h3 className="font-black text-xs uppercase tracking-widest text-[#c8952e]">{selectedProd.nombre}</h3>
                ...
                <TableBody>
                   {movs.map(m => (
                      <TableRow key={m.id} className="border-b border-line/30">
                         ...
                         <TableCell className={`text-center font-black ${m.cantidad > 0 ? 'text-green-600' : 'text-red-500'}`}>{m.cantidad > 0 ? '+' : ''}{m.cantidad}</TableCell>
        ...
        <div className="modal-body p-6 space-y-4 bg-white">
          <div className="grid grid-cols-2 gap-4">
             ...
             <div className="form-group"><label className="text-[10px] font-black uppercase text-ink/60 mb-1 block">Cantidad</label>
               <Input className="h-10 text-center font-black" type="text" value={cantidad} onChange={e => setCantidad(e.target.value)} />
             </div>
          </div>
      ...
      <div className="modal-box bg-white max-w-2xl border-2 border-line rounded-xl overflow-hidden shadow-2xl">
        <div className="modal-head py-4 px-6 border-b border-line bg-ink flex justify-between items-center text-white">
          <h3 className="font-black uppercase italic tracking-tighter text-sm flex items-center gap-2">
            <Box className="w-5 h-5 text-[#c8952e]" /> {producto ? 'EDITAR FICHA' : 'NUEVO ÍTEM / PRODUCTO'}
          </h3>
          <button onClick={onClose} className="text-white/40 hover:text-white"><X className="w-5 h-5"/></button>
        </div>

        <div className="flex bg-surface-soft border-b border-line">
          <button onClick={() => setActiveTab('general')} className={`flex-1 py-3 text-[10px] font-black uppercase tracking-widest border-b-2 transition-all ${activeTab === 'general' ? 'border-[#c8952e] text-[#c8952e] bg-white' : 'border-transparent text-ink/40'}`}>General</button>
          <button onClick={() => setActiveTab('precios')} className={`flex-1 py-3 text-[10px] font-black uppercase tracking-widest border-b-2 transition-all ${activeTab === 'precios' ? 'border-[#c8952e] text-[#c8952e] bg-white' : 'border-transparent text-ink/40'}`}>Precios</button>
          <button onClick={() => setActiveTab('kit')} className={`flex-1 py-3 text-[10px] font-black uppercase tracking-widest border-b-2 transition-all ${activeTab === 'kit' ? 'border-[#c8952e] text-[#c8952e] bg-white' : 'border-transparent text-ink/40'}`}>Kits / Combos</button>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-4">
                <div className="space-y-1">
                  ...
                  <Input className="h-10 font-black text-ink" value={datos.codigo} onChange={e => setDatos({...datos, codigo: e.target.value})} placeholder="00000000" autoFocus />
                </div>
                <div className="space-y-1">
                  <label className="text-[10px] font-black uppercase text-ink/50 block">Nombre del Producto</label>
                  <Input className="h-10 font-black text-ink uppercase" value={datos.nombre} onChange={e => setDatos({...datos, nombre: e.target.value})} />
                </div>
                <div className="space-y-1">
                  <div className="flex justify-between items-center mb-1">
                    <label className="text-[10px] font-black uppercase text-ink/50">Categoría</label>
                    <div className="flex gap-1">
                      <button onClick={() => handleAddListItem('categorias')} className="text-[#c8952e]"><PlusCircle className="w-3.5 h-3.5"/></button>
                      <button onClick={() => handleRemoveListItem('categorias', datos.categoria)} className="text-red-500"><MinusCircle className="w-3.5 h-3.5"/></button>
                    </div>
                  </div>
              ...
              <div className="space-y-4">
                 <div className="grid grid-cols-2 gap-4">
                   ...
                   <div className="p-3 bg-red-500/5 border border-red-500/20 rounded-xl text-center">
                     <label className="text-[9px] font-black uppercase text-red-500/70 block mb-1">Mínimo</label>
                     <Input className="bg-transparent border-none text-center font-black text-xl w-full text-red-500 focus:outline-none" value={datos.stockMinimo} onChange={e => setDatos({...datos, stockMinimo: e.target.value})} />
                   </div>
                 </div>
                 <div className="flex items-center gap-3 p-4 bg-surface-soft rounded-xl border border-line">
                   <button onClick={() => setDatos({...datos, aplicaIVA: !datos.aplicaIVA})} className={`w-12 h-6 rounded-full transition-all relative ${datos.aplicaIVA ? 'bg-green-600' : 'bg-ink/20'}`}><div className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${datos.aplicaIVA ? 'right-1' : 'left-1'}`} /></button>
          ...
          {activeTab === 'precios' && (
            <div className="space-y-6">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 bg-surface-soft p-5 rounded-2xl border border-line shadow-inner">
                <div className="space-y-1"><label className="text-[9px] font-black uppercase text-ink/50">Costo ($)</label><Input className="h-12 font-black text-lg" value={datos.costoUSD} onChange={e => setDatos({...datos, costoUSD: e.target.value})} /></div>
                <div className="space-y-1"><label className="text-[9px] font-black uppercase text-[#c8952e]">Margen %</label><Input className="h-12 font-black text-lg text-[#c8952e]" value={datos.margen} onChange={e => recalcularTridireccional('margen', e.target.value)} /></div>
                <div className="space-y-1"><label className="text-[9px] font-black uppercase text-green-600">Venta ($)</label><Input className="h-12 font-black text-lg text-green-600" value={datos.precioUSD} onChange={e => recalcularTridireccional('precioUSD', e.target.value)} /></div>
                <div className="space-y-1"><label className="text-[9px] font-black uppercase text-ink">Venta (BS)</label><Input className="h-12 font-black text-lg" value={datos.precioBS} onChange={e => recalcularTridireccional('precioBS', e.target.value)} /></div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                 <div className="space-y-1"><label className="text-[9px] font-black uppercase text-ink/40">P. Mayor ($)</label><Input className="h-10 font-bold" value={datos.precioMayorUSD} onChange={e => setDatos({...datos, precioMayorUSD: e.target.value})} /></div>
                 <div className="space-y-1"><label className="text-[9px] font-black uppercase text-ink/40">P. Descuento ($)</label><Input className="h-10 font-bold" value={datos.precioOfertaUSD} onChange={e => setDatos({...datos, precioOfertaUSD: e.target.value})} /></div>
                 <div className="space-y-1"><label className="text-[9px] font-black uppercase text-ink/40">P. Promo ($)</label><Input className="h-10 font-bold" value={datos.precioPromoUSD} onChange={e => setDatos({...datos, precioPromoUSD: e.target.value})} /></div>
              </div>
            </div>
          )}
            ...
            <div className="space-y-6">
              <div className="flex flex-col gap-4 p-4 bg-ink text-white rounded-xl">
                <div className="flex items-center gap-3">
                  <button onClick={() => setDatos({...datos, isKit: !datos.isKit})} className={`w-12 h-6 rounded-full transition-all relative ${datos.isKit ? 'bg-[#c8952e]' : 'bg-white/20'}`}><div className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${datos.isKit ? 'right-1' : 'left-1'}`} /></button>
                ...
                {datos.isKit && (
                  <div className="flex gap-6 border-t border-white/10 pt-4">
                    ...
                    <div className="flex items-center gap-2"><input type="radio" checked={datos.kitType === 'stock_componentes'} onChange={() => setDatos({...datos, kitType: 'stock_componentes'})} /><label className="text-[9px] font-black uppercase">Depende de Componentes</label></div>
                  </div>
                )}
              </div>
              {datos.isKit && (
                <div className="space-y-4">
                  <div className="relative"><Search className="absolute left-3 top-3 w-4 h-4 text-ink/30" /><Input className="h-12 pl-10 text-xs font-black uppercase" placeholder="Buscar productos..." value={kitSearch} onChange={e => setKitSearch(e.target.value)} />{filteredProdsForKit.length > 0 && (<div className="absolute top-full left-0 right-0 bg-white border border-line rounded-lg shadow-2xl z-50 mt-1 overflow-hidden">{filteredProdsForKit.map(pk => (<div key={pk.id} onClick={() => { setDatos({...datos, kitItems: [...datos.kitItems, { productoId: pk.id, nombre: pk.nombre, cantidad: 1 }]}); setKitSearch(''); }} className="p-3 border-b border-line hover:bg-[#c8952e]/10 cursor-pointer flex justify-between items-center"><span className="text-xs font-black uppercase">{pk.nombre}</span><Plus className="w-4 h-4 text-[#c8952e]"/></div>))}</div>)}</div>
                  <Card className="border-line shadow-sm overflow-hidden"><div className="table-wrap"><Table><TableHeader className="bg-surface-soft"><TableRow><TableHead className="text-[10px] font-black uppercase">Componente</TableHead><TableHead className="text-[10px] font-black uppercase text-center">Cant</TableHead><TableHead></TableHead></TableRow></TableHeader><TableBody>
                    {datos.kitItems.map((ki: KitItem, index: number) => (
                      <TableRow key={index} className="border-b border-line/30"><TableCell className="text-[11px] font-black uppercase text-ink">{ki.nombre}</TableCell><TableCell className="text-center"><Input className="w-12 h-8 text-center font-black bg-surface-soft border-line inline-block" type="number" value={ki.cantidad} onChange={e => { const n = [...datos.kitItems]; n[index].cantidad = parseInt(e.target.value) || 1; setDatos({...datos, kitItems: n}); }} /></TableCell><TableCell className="text-center"><button onClick={() => setDatos({...datos, kitItems: datos.kitItems.filter((_:any, i:number) => i !== index)})} className="text-red-500"><Trash2 className="w-4 h-4"/></button></TableCell></TableRow>
                    ))}
                    {datos.kitItems.length === 0 && (<TableRow><TableCell colSpan={3} className="py-10 text-center text-ink/20 font-black uppercase italic text-[10px]">Sin componentes</TableCell></TableRow>)}
        ...
        <div className="modal-foot p-5 bg-surface-soft border-t border-line flex justify-end gap-3"><Button variant="secondary" className="px-8 font-black uppercase text-[10px]" onClick={onClose}>Cerrar</Button><Button className="bg-[#c8952e] hover:bg-[#d9a540] text-black px-10 font-black uppercase text-[10px] shadow-lg" onClick={handleSave}>{producto ? 'ACTUALIZAR' : 'CREAR PRODUCTO'}</Button></div>
      </div>
    </div>
  );
}
